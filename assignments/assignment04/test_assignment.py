...
def test_add():
    m = load_student_work()
    assert m.add(2, 3) == 5       # c left out -> defaults to 0
    assert m.add(2, 3, 4) == 9    # all three added


def test_greet():
    m = load_student_work()
    assert m.greet("Sam") == "Hello, Sam!"          # default greeting
    assert m.greet("Sam", "Hi") == "Hi, Sam!"       # custom greeting


def test_triple():
    m = load_student_work()
    assert m.triple(5) == 15
